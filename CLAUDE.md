...
El atributo `Secure` de la cookie **no** se decide con `NODE_ENV` (rompería la sesión en cuanto se accede por HTTP plano, el caso típico de un NAS en la LAN sin TLS: el navegador descartaría la cookie tras el login, y la primera petición protegida real —p. ej. importar— daría 401). Se calcula por petición en `cookieOptions(req)` (`modules/auth/auth.controller.ts`) a partir de `req.secure`, salvo que `COOKIE_SECURE` fuerce `true`/`false`. `app.set('trust proxy', 1)` en `app.ts` hace que `req.secure` respete `X-Forwarded-Proto` si hay un proxy TLS delante.
